...
pts = []  # 좌표를 저장하는 리스트
cnt = 0 # 상태를 추적하기 위한 변수(안/밖)
is_inside = False
id_count = {} # ID별로 다각형에 진입한 횟수를 추적
prevent = False
...
def draw_boxes(img, bbox, identities=None, categories=None, 
    ...
    global is_inside, pts, cnt, prevent, inside_polygon, track_ids
...
def main(opt):
    ...
    if source != '0':
        ...
        file_path = f"roi_coor/webcam.txt"
        if os.path.isfile(file_path):
            with open(file_path, "r") as file:
                lines = file.readlines()
                pts = [tuple(map(int, line.strip().split(','))) for line in lines]
        ...
        while (saved==False):
            ...
            cv2.setMouseCallback('Webcam', draw_roi)
            key = cv2.waitKey(1) & 0xFF
    ...
    if roi_confirmed==True or source == '0':
...
if __name__ == "__main__":
    ...
    print(f"프레임을 읽는 데 걸린 시간: {elapsed_time} 초")
